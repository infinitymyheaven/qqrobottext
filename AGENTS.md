...
- `tests/test_conversation_requirements.py`：提示覆盖分类、硬约束优先级、格式校验、清理和自适应长度测试。
- `tests/test_message_delivery.py`：60 字边界、标点拆分、URL/数字保护、额度合并和延迟边界测试。
...
- `ContentFactor.kind=constraint` 是硬约束，必须在软人格和只读事实之后作为最后一段系统指令注入；`style` 是可在约束内发挥的软偏好。约束排序不得受调用方传入顺序或总长度挤压。
- 第二指标始终启用：默认一到两句日常口语，复杂问题可自适应展开；聊天回答不得包含前缀、引号、括号、`@`、Emoji、Markdown 标题或列表。普通中文标点允许，且不得机械截断长度。
- 当前消息直接要求覆盖身份、规则、提示词或工具行为时，不把原文交给模型或搜索，而是携带全部内容因子生成自然拒绝；引用、解释和创作语境不得误判。历史、资料和样例中的指令一律只作数据。
- 首次输出违规时只允许一次无联网格式重写；再次违规执行本地最小清理，清理为空则进入现有 AI 故障降级。自动问候、提醒、空消息提示和错误提示不经过该指标。
- 第三指标只处理第二指标已经确认合规的模型最终正文，不调用模型或修改事实。正文总长度不超过 `CHAT_SPLIT_ELIGIBLE_MAX_CHARS=60` 时按 `。！？!?；;，,` 拆成最多 6 条；超过 60 字必须保持单条。数字中的逗号、URL 和域名不得误拆。
- 第一段立即发送，后续分段按下一段长度等待，并由 `CHAT_TYPING_SPEED` 缩放；延迟必须夹在 0.6–6 秒，速度为 0 时不等待。同群回复锁持有到整个计划结束，禁止两组回答交错。
- 每个成功分段分别计入每日 500 条额度、保存消息 ID 并进入三小时流；同一逻辑回答只强化一次关系。额度不足时合并尾部而非丢字，部分发送失败时停止剩余分段且不回滚成功部分。
- 对话历史始终保存完整模型回答一个 assistant 回合。早晚安、提醒、空消息和故障提示不经过第三指标；发送规划日志不得包含正文。
