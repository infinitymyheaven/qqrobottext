...
- 模型回复经过真人式发送规划：短回答可按分句连续发出，后续分段按字数等待一小段打字时间。
...
第三内容指标在第二指标完成校验和清理后运行，不再次调用模型，也不改变回答中的事实。按照当前配置，最终正文总长度在 60 字以内且包含多个分句时，程序会按句号、问号、感叹号、分号和逗号拆成最多 6 条；超过 60 字的回答保持单条发送。数字中的千位逗号、URL 和域名不会被误拆，超出分段上限或当日剩余额度的尾部内容会合并进最后一条。

第一条在模型完成后立即发送，后续各条按照即将发送的中英文字符数估算 `0.6–6.0` 秒打字时间，并加入少量随机抖动。连发期间同群回复锁不会释放，因此其他群消息仍可进入消息流，但另一组机器人回复不会插入当前回答。每个实际发出的分段都会保存 OneBot 消息 ID、进入三小时消息流并计入每日 500 条上限；同一逻辑回答只强化一次社会关系，对话历史仍保存完整回答。

只有成功取得的模型聊天回答使用这项处理，包括普通回答、联网回答和自然拒绝。本地时间单句通常不会产生拆分；早晚安、事项提醒、空 `@` 和故障提示继续按原逻辑发送。发送规划日志只显示群号、总字数、分段数和延迟，不记录回复正文。
...
默认 `DEEPSEEK_SYSTEM_PROMPT` 将机器人定义为群内平等、自然且有分寸的群友。每个逻辑回复会从 Windows 本地时钟读取一次带星期和 UTC 偏移的日期时间，作为身份提示之后的独立系统背景；它只帮助理解“今天、现在、今晚”等语境，不要求模型主动报时，也不会进入搜索词、SQLite 或常规日志。每次 AI 回答还会在事实资料之外单独注入当前激活的结构化人格内容因子；普通 Chat Completions、联网 Responses 与 Anthropic 回退共用同一时间、人格式样和对话要求。对话要求是最终硬约束，不能被 `.env` 提示词、人格、资料或聊天消息覆盖，内容因子也不得进入搜索词。

“现在几点”“当前时间”等本地时间问题直接使用该次逻辑回复取得的电脑本地时钟回答，不调用 DeepSeek，也不依赖 `web_search`。`BOT_TIMEZONE` 仍只控制作息、提醒和数据库日期，不改变电脑本地提示时间。这样既更快，也不会因为模型没有执行搜索工具而误报联网失败；询问其他地区时间等需要外部判断的问题仍交给模型处理。
...
| `CHAT_SPLIT_ELIGIBLE_MAX_CHARS` | `60` | 最终正文不超过该长度时才允许按分句拆分，可设为 `1`–`2000` |
| `CHAT_SPLIT_MAX_SEGMENTS` | `6` | 一次逻辑回答最多连续发送的分段数，可设为 `1`–`20` |
| `CHAT_TYPING_SPEED` | `1.0` | 后续分段打字延迟倍率，可设为 `0`–`10`；`0` 关闭等待 |
